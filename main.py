from tkinter import (
    ...
    Button, LEFT, Frame, messagebox, filedialog
    )
from enum import Enum
from typing import Final
import platform
from global_val import CYRILLIC, ENGLISH, NUMBERS
from logic import (
...
class App:
    ...
    def _get_valid_plane_text(self, text: str) -> str:
        allowed_chars: str
        if self.encription_method.get() == CipherMethod.DECIMATION.value:
            allowed_chars = ENGLISH
        else:
            allowed_chars = CYRILLIC
    ...
    def plane_text(self) -> str:
        text: str = self.entry_plane_text.get().strip()
        return self._get_valid_plane_text(text)
    ...
    def _on_cipher(self) -> None:
        key: str = self.key
        text: str = self.plane_text

        if not key or not text:
            messagebox.showwarning("Внимание", "Введите ключ и текст для шифровки.")
            return
        ...
        if self.encription_method.get() == CipherMethod.VIGENERE.value:
            ...
            l = len(text)
            if step > l:
                messagebox.showerror("Ошибка", f"Шаг больше самого текста.")
                return            
            if gcd(l, step) != 1:
                messagebox.showerror("Ошибка", f"НОД({l}, {step}) должен быть равен 1.")
                return
            else:
    ...
    def _on_decipher(self) -> None: 
        ...
        if not key or not text:
            messagebox.showwarning("Внимание", "Введите ключ и текст для дешифровки!")
            return
